...
インフラエンジニア,クラウド,初級,クラウドセキュリティ基礎,IAMポリシー・ロール設定、MFA有効化、セキュリティグループ・NACLの基本設定ができる
インフラエンジニア,クラウド,初級,クラウドバックアップ/復旧,自動バックアップ設定・スナップショット管理、基本的なデータ復旧手順・RTO/RPO理解ができる
...
インフラエンジニア,ネットワーク,初級,DNS基礎設定,DNSレコード（A・CNAME・MX等）設定、ドメイン管理・TTL設定、nslookup・dig等のDNS診断ができる
インフラエンジニア,ネットワーク,初級,ネットワークプロトコル基礎,TCP/IP・HTTP/HTTPS・DHCP・ARP等の基本プロトコル理解・パケットキャプチャ基礎ができる
...
開発エンジニア（プログラマー）,プログラミング,中級,コードレビュー/品質向上,効果的なコードレビュー実施・静的解析ツール活用・技術的負債特定・コード品質指標管理ができる
...
開発エンジニア（SE）,データベース,中級,データマイグレーション,大量データ移行戦略・移行ツール選定・データ整合性確保・移行テスト・ロールバック計画策定ができる
...
マネジメント,マネジメント,中級,人材育成/評価,メンバースキル評価・育成計画策定・メンタリング実施・パフォーマンス評価・キャリア相談対応ができる
...
マネジメント,マネジメント,上級,経営層とのコミュニケーション,経営会議参加・経営層レポーティング・経営方針現場展開・経営層向け資料作成ができる`;
    ...
    // 各レコードにIDを付与する（ソート前の順番で）
    const recordsWithId = records.map((record: any, index: number) => ({
      ...record,
      id: index + 1,
    }));
    
    // IDを付与した後で、分野ごとにソートする
    const sortedRecords = sortSkillsByDomain(recordsWithId);
...
/**
 * スキルデータを分野ごとに正しい順序でソートする関数
 * @param skills スキルデータの配列
 * @returns ソート済みのスキルデータ
 */
export const sortSkillsByDomain = (skills: Skill[]) => {
  const domainOrder = [
    "インフラエンジニア",
    "開発エンジニア（プログラマー）", 
    "開発エンジニア（SE）",
  ...
  return skills.sort((a, b) => {
    // まず分野で並び替え
    const domainIndexA = domainOrder.indexOf(a.分野);
    const domainIndexB = domainOrder.indexOf(b.分野);
    ...
    // 同じ分野内では項目で並び替え
    if (a.項目 !== b.項目) {
      return a.項目.localeCompare(b.項目, 'ja');
    }
    
    // 同じ項目内ではレベルで並び替え
    const levelIndexA = levelOrder.indexOf(a.レベル);
    const levelIndexB = levelOrder.indexOf(b.レベル);
