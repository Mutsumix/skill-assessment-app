# 技術マップ - スキル習得状況可視化アプリ 設計書

## 1. アプリケーション概要

このアプリケーションは、社員のスキル習得状況を可視化するためのスマホアプリです。128 項目のスキル評価を通じて、技術レベルを正確に把握・可視化できます。シンプルな解説文で迷わず回答でき、履歴管理機能により継続的なスキル成長を追跡します。AsyncStorage によるデータ永続化でユーザー体験を向上させています。
...
- CSV ファイルに記載された 128 のスキル項目を評価（シンプルな解説文で迷わず回答）
...
0. **スプラッシュ画面（SplashScreen）**

   - アプリアイコンを使用した魅力的な起動画面
   - "技術マップ"ブランド名表示
   - プロフェッショナルな視覚効果
...
3. **スキル評価画面（AssessmentScreen）**
   ...
   - 128 項目のスキルをスワイプで評価
   - シンプルな解説文で迷わず回答
   - 分野別進捗バー表示（インフラ・プログラマー・SE・マネジメント）
   - 全体進捗パーセンテージ表示
   - 分野ごとに休憩カードを表示
   - 手動保存機能・ホーム戻り機能
